...
import type { AssistantMessageEntry, UserMessageEntry, UserMessageImage } from '../../lib/types';
import { cn } from '../../lib/utils';
import { MessageResponse } from '../ai-elements/markdown';
...
export function UserBubble({ entry, isLastUserMessage }: UserBubbleProps) {
  ...
  return (
    <div className="flex justify-end" {...(isLastUserMessage ? { 'data-last-user-message': 'true' } : {})}>
      <div className="max-w-[85%] sm:max-w-[70%]">
        {/* 图片附件 */}
        {entry.images && entry.images.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-2 justify-end">
            {entry.images.map((img, i) => (
              <ImageThumbnail key={`${entry.id}-img-${i}`} image={img} />
            ))}
          </div>
        )}
        {/* 文本内容 — 品牌色淡底 + 折叠 */}
        {entry.content && (
          <div className="relative rounded-2xl rounded-br-md bg-user-bubble border border-user-bubble-border overflow-hidden">
            <div
              ref={contentRef}
              className="px-5 py-3 text-sm text-white whitespace-pre-wrap font-display leading-relaxed"
...
export function AssistantBubble({ entry }: AssistantBubbleProps) {
  return (
      ...
      <div className="flex-1 min-w-0 space-y-4">
        ...
        {entry.chunks.map((chunk, i) => {
          if (chunk.type === 'thought') {
            return (
              <Reasoning key={`${entry.id}-chunk-${i}`}>
                <ReasoningTrigger />
                <ReasoningContent>
                  <div className="text-sm text-text-secondary leading-relaxed whitespace-pre-wrap">{chunk.text}</div>
                </ReasoningContent>
              </Reasoning>
            );
          }
          return (
            <div key={`${entry.id}-chunk-${i}`} className="message-content text-text-primary leading-[1.75]">
...
function ImageThumbnail({ image }: { image: UserMessageImage }) {
  ...
  return (
    <>
      ...
      >
        <img src={dataUrl} alt="用户上传的图片" className="h-20 w-20 object-cover" />
      </button>
      {zoomed && (
        <button
          type="button"
        ...
        >
          <img src={dataUrl} alt="用户上传的图片" className="max-h-full max-w-full rounded-lg" />
        </button>
      )}
    </>
  );
...
// =============================================================================
// Claude 星形标志（与 RCS Web UI 同源的 path）
// =============================================================================
